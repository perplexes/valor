...
Scene = (function() {
  ...
  function Scene(game, client) {
    ...
    this.starfield = new Starfield(layers["Starfield"], this.viewport);
    this.views = {};
  }
  ...
  Scene.prototype.step = function(game, timestamp, delta_s) {
    ...
    game.staticGraph.searchExpand(this.viewport._extent, 16, 16, this.updateEntity, this);
    game.dynamicGraph.searchExpand(this.viewport._extent, 16, 16, this.updateEntity, this);
    ...
    for (view = _j = 0, _len1 = _ref1.length; _j < _len1; view = ++_j) {
      ...
      if (!view.displayed) {
        view.remove();
        _results.push(delete views[hash]);
      } else {
  ...
  Scene.prototype.updateEntity = function(entity) {
    ...
    view = (_base = this.views)[_name = entity.hash] || (_base[_name] = View.build(entity));
    updated = view.update(this.viewport);
    if (!updated) {
      view.remove();
      return delete views[entity.hash];
    }
  };
