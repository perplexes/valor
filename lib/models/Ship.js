...
Ship = (function(_super) {
  ...
  function Ship(simulator, player, options) {
    ...
    loc = this.locations[options.location] || this.locations.nearSafe;
    pos = null;
    if (options.pos) {
      pos = options.pos;
    } else {
    ...
    Ship.__super__.constructor.call(this, simulator, pos, new Vector2d(0, 0), 32, 32);
    this.player = player;
  ...
  Ship.prototype.simulate = function(delta_s) {
    var bullet, _i, _len, _ref;
    this.vel.clamp(this.velClamp);
    if (this.gunTimeout > 0) {
      this.gunTimeout -= delta_s;
    }
    Ship.__super__.simulate.call(this, delta_s);
